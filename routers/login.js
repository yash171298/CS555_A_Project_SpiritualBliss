...
router.post('/', async (req, res) => {
    try {
        ...
        if (checkEmail.result)  email = (checkEmail.value).toLowerCase()
            ...
            res.status(400).render('posts/login', { error: "Incorrect user input", "message":"", title: "Mental and physical health-Yoga  - Login", notFound: false, userData:null})
            return;
        } 
        ...
        if (logInUser && logInUser.userData!=null) {            
            ...
            res.status(404).render('posts/login', { error: "Incorrect email or password!", title: "Mental and physical health-Yoga  - Login",  notFound: false });
        }
        
    } catch (e) {
        res.status(e.statusCode?e.statusCode:500).render('posts/login', { error: e.error, title: "Mental and physical health-Yoga  - Login",  notFound: false })
    }
});
